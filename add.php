...
<body>
    ...
    if( isset($_POST['Submit'])){
        ...
        if( empty($iso) || empty($name) || empty($nicename) || empty($iso3) || empty($numcode) || empty($phonecode)){
            if(empty($iso)){
                echo "<font color='black' > ISO field is empty. </font><br/>";
            }
            if(empty($name)){
                echo "<font color='black' > Name field is empty. </font><br/>";
            }
            if(empty($nicename)){
                echo "<font color='black' > Nice Name field is empty. </font><br/>";
            }
            if(empty($iso3)){
                echo "<font color='black' > ISO3 field is empty. </font><br/>";
            }
            if(empty($numcode)){
                echo "<font color='black' > Number Code field is empty. </font><br/>";
            }
            if(empty($phonecode)){
                echo "<font color='black' > Phone Code field is empty. </font><br/>";
            }
